...
class XLWCCT_Admin {
	...
	public function wcct_enqueue_admin_assets() {
		...
		if ( in_array( $page, [ 'xl-cart', 'xl-checkout', 'xl-automations', 'xl-payments' ], true ) ) {
			wp_enqueue_style( 'xlwcty_admin-css', $this->get_admin_url() . 'assets/css/xl-submenu-pages.css', XLWCCT_VERSION );
			wp_enqueue_script( 'xl-installer-js', $this->get_admin_url() . 'assets/js/xl-addon-installer.js', XLWCCT_VERSION );
			$xl_installer_data['nonce'] = wp_create_nonce( 'xl_addon_installation_nonce' );
			wp_localize_script( 'xl-installer-js', 'xl_installer_data', array(
				'ajaxUrl' => admin_url( 'admin-ajax.php' ),
				'nonce'   => $xl_installer_data['nonce'],  // Add nonce to xl_installer_data
			) );
		}
		if ( 'wc-settings' === $page && 'xl-countdown-timer' === filter_input( INPUT_GET, 'tab' ) && 'blank' === get_option( 'xlp_is_opted', 'blank' ) ) {
			wp_enqueue_style( 'xlo-optin-css', $this->get_admin_url() . 'assets/css/xlo-optin.css', XLWCCT_VERSION );
			wp_enqueue_script( 'xlo-optin-js', $this->get_admin_url() . 'assets/js/xlo-optin.js', XLWCCT_VERSION );
	...
	public function maybe_show_advanced_update_notification() {
		...
		if ( ! ( 'plugins.php' == $screen->parent_file || 'index.php' == $screen->parent_file || WCCT_Common::get_wc_settings_tab_slug() == filter_input( INPUT_GET, 'tab' ) ) ) {
			return;
		}

		// option to permanently hide the update notice
		$hide_notice = get_option( 'xlplugin_finale_lite_hide_update_notice', 'no' );
        ...
        <div class="xlplugin-notice-message notice notice-warning is-dismissible">
            <a class="xlplugin-message-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'xlplugin-finale-lite-update-notice', 'hide' ), 'xlplugin_finale_lite_update_notice_nonce', '_xlplugin_finale_lite_update_notice_nonce' ) ); ?>">
				<?php esc_html_e( 'Dismiss', 'finale-woocommerce-sales-countdown-timer-discount' ); ?>
            </a>
            <p>
				<?php
	...
	/**
	 * Set option to hide xl plugin finale lite update notice
	 */
	public static function hide_plugins_update_notices() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( $_GET['_xlplugin_finale_lite_update_notice_nonce'] ), 'xlplugin_finale_lite_update_notice_nonce' ) ) {
			wp_die( __( 'Action failed. Please refresh the page and retry.', 'finale-woocommerce-sales-countdown-timer-discount' ) );
		}
		update_option( 'xlplugin_finale_lite_hide_update_notice', 'yes' );
		$redirect_link = add_query_arg( array(
			'page' => 'wc-settings',
			'tab'  => 'xl-countdown-timer',
		), admin_url( 'admin.php' ) );
